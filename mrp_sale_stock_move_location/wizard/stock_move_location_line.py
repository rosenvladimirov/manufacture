...
class SaleStockMoveLocationWizardLine(models.TransientModel):
    _name = "wiz.sale.stock.move.location.line"
    _description = 'Wizard for collect bom data'

    move_location_wizard_id = fields.Many2one(
        string="Move location Wizard",
        comodel_name="wiz.sale.stock.move.location",
        ondelete="cascade",
        required=False,
        index=True,
    )
    ...
    all_wh_total_quantity = fields.Float(
        string="Total Availability Quantity in all Warehouse",
        digits=dp.get_precision('Product Unit of Measure'),
    )
    ...
    purchase_ids = fields.Many2many(
        ...
        relation='wiz_sale_purchase_line',
        column1='wiz_line_id',
    ...
    #     for record in self:
    #         if record.product_id.product_tmpl_id.categ_id.product_type_id:
    #             record.product_type_id = record.product_id.product_tmpl_id.categ_id.get_product_type_id()
    ...
    def onchange_product_id(self):
        if self.move_location_wizard_id.work_with_reservation:
            ...
            search_exclude_args = [
                ('location_id', '=', wiz.destination_location_id.id),
                ('product_id', '=', self.product_id.id),
            ]
            if self.lot_id:
                search_exclude_args.append(('lot_id', '=', self.lot_id.id))
            else:
                search_exclude_args.append(('lot_id', '=', False))
            if self.package_id:
                search_exclude_args.append(('package_id', '=', self.package_id.id))
            res = self.env['stock.quant'].read_group(search_exclude_args, ['quantity'], [])
            exclude_product_uom_qty = res[0]['quantity']
            self.exclude_product_uom_qty = exclude_product_uom_qty
            self.location_id = wiz.origin_location_id
            self.location_dest_id = wiz.destination_location_id
    ...
    def _get_move_line_values(self, picking, move):
        self.ensure_one()
        qty_todo, qty_done, exclude_product_uom_qty = self._get_available_quantity()
        return {
            "product_id": self.product_id.id,
    ...
    def _get_available_quantity(self):
        ...
        if self.env.context.get("planned"):
            ...
            return self.product_uom_qty, self.real_product_uom_qty, 0
        search_args = [
            ('location_id', '=', self.location_id.id),
            ('product_id', '=', self.product_id.id),
        ]
        ...
        if self.lot_id:
            ...
            search_args.append(('lot_id', '=', False))
        res = self.env['stock.quant'].read_group(search_args, ['quantity'], [])
        available_dest_qty = res[0]['quantity']
        ...
        available_qty_lt_move_qty = self._compare(
            available_qty, self.real_product_uom_qty, rounding) == -1
        if available_qty_lt_move_qty:
            return available_qty, self.real_product_uom_qty, available_dest_qty
        return 0, self.real_product_uom_qty, available_dest_qty


class SaleWizardLine(models.TransientModel):
    _name = "wiz.sale.line"
    _description = 'Wizard for collect bom lines'
...
class SaleLineWizardLine(models.TransientModel):
    _name = "wiz.sale.order.line"
    _description = 'Wizard for collect sale order lines'
